...
/**
 * Introspector for persistence class construction/instantiation/building mechanism. Metadata of the
 * introspected classes will be cached for future use.
 *
 * @author Sai Pullabhotla
 */
public class ConstructorIntrospector {

  /** Method name for producing an instance of Persistence class from the corresponding Builder */
  public static final String METHOD_NAME_BUILD = "build";
  /** First of the supported method names to produce a new Builder for a persistence class */
  public static final String METHOD_NAME_NEW_BUILDER = "newBuilder";
  /** Second/Last of the supported method names to produce a new Builder for a persistence class */
  ...
  /** Valid/supported method names for getting a new Builder for a Persistence class */
  private static final String[] NEW_BUILDER_METHOD_NAMES = {
    METHOD_NAME_NEW_BUILDER, METHOD_NAME_BUILDER
  };
  ...
  /**
   * Introspects the given Persistence class and returns the {@link ConstructorMetadata}.
   *
   * @param clazz the class to introspect
  ...
  private void process() {
    ...
    if (mh != null) {
      metadataBuilder
          .setConstructionStrategy(ConstructorMetadata.ConstructionStrategy.CLASSIC)
    ...
    if (mh == null) {
      final String pattern =
          "Class %s requires a public no-arg constructor or a public static "
              + "method that returns a corresponding Builder instance. The name of the static method "
              + "can either be %s or %s";
      String error =
          String.format(pattern, clazz.getName(), METHOD_NAME_NEW_BUILDER, METHOD_NAME_BUILDER);
      throw new UnsupportedConstructionStrategyException(error);
    }
    Class<?> builderClass = mh.type().returnType();
    metadataBuilder
        .setConstructionStrategy(ConstructorMetadata.ConstructionStrategy.BUILDER)
        .setConstructorMethodHandle(mh)
        .setBuilderClass(builderClass);
    MethodHandle buildMethodHandle =
        IntrospectionUtils.findInstanceMethod(builderClass, METHOD_NAME_BUILD, clazz);
